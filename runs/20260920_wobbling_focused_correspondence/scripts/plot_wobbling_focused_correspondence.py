...
box(a,.01,.33,.48,.44,'#e7f2f5');box(a,.52,.33,.47,.44,'#f7eddf')
a.text(.035,.715,'PRIMARY  |  C1 Balance / Ataxia',fontsize=17,fontweight='bold')
a.text(.035,.635,'Ataxia\nBalance disorder\nCoordination abnormal\nGait disturbance',va='top',fontsize=13,linespacing=1.6)
a.text(.545,.715,'SECONDARY  |  C5 Motor weakness',fontsize=17,fontweight='bold')
a.text(.545,.615,'Muscular weakness\nHypotonia',va='top',fontsize=14,linespacing=1.7)
a.text(.545,.40,'Possible motor contributor; not identical to C1',fontsize=12,color='#654f36')
for xx,title,terms,role in [(.01,'CONTEXT  |  C3 Sedation','Somnolence / Sedation','contributing / potentially confounding phenotypes'),(.35,'CONTEXT  |  C4 Dizziness','Dizziness / Vertigo','contributing / potentially confounding phenotypes'),(.69,'CONTEXT  |  C2 Fall','Fall','downstream outcome')]:
 box(a,xx,.005,.30,.295,'#f3f4f6');a.text(xx+.013,.245,title,fontsize=13,fontweight='bold');a.text(xx+.013,.175,terms,fontsize=12)
 role=role.replace(' / potentially confounding phenotypes',' / potentially\nconfounding phenotypes')
 a.text(xx+.013,.115,role,fontsize=10.5,va='top',linespacing=1.2)
...
b.text(5/6,1.013,'SECONDARY: C5',transform=b.transAxes,ha='center',fontsize=13,fontweight='bold',color='#845a21')
matrix_plot(e,ctx,nctx,[AXIS[t]+' '+t for t in CONTEXT],IDS)
e.set_title('E  |  Context phenotypes\nnot primary wobbling endpoint',loc='left',fontsize=17,fontweight='bold',pad=40)
e.text(.5,1.013,'CONTEXT ONLY: C3 / C4 / C2',transform=e.transAxes,ha='center',fontsize=13,color='#56616b')
