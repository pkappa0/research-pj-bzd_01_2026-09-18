# Label revision requested by the user

元run `runs/20260920_wobbling_focused_correspondence/` の数値・表・feature/term順を保持した文言改訂。Panel AはPRIMARY PHENOTYPE / RELATED MOTOR PHENOTYPE / RELATED CLINICAL PHENOTYPESに変更し、B/Eの見出しも統一。元図は上書きしない。以下のmethodsの数値処理は元runと同一。
...
- **A — Clinical phenotype definition**: PRIMARYはC1 Ataxia / Balance disorder / Coordination abnormal / Gait disturbance。RELATED MOTOR PHENOTYPEはC5 Muscular weakness / Hypotoniaで、C1と同一視しない。C3 Sedation、C4 Dizziness、C2 Fallは「RELATED CLINICAL PHENOTYPES」として表示する。contributing/confounding/downstreamという役割付けは本改訂図から削除。これは概念整理であり因果順序を検証したものではない。矢印は使用しない。
...
- **E — Related clinical phenotypes — not primary wobbling endpoint**: C3 Somnolence、C3 Sedation、C4 Dizziness、C4 Vertigo、C2 Fallの順。Bとは別のheatmapで、同じ48 feature・既存rho・n_effectiveを示す。primary C1/C5と混ぜない。C6は本図に表示しないが元Y snapshotには保持する。
...
```bash
python runs/20260920_wobbling_related_phenotypes/scripts/plot_wobbling_focused_correspondence.py
```
