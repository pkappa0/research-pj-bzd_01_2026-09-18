...
import pandas as pd, json,hashlib,datetime,subprocess
R=Path(__file__).resolve().parents[1];root=R.parents[1]
s=json.loads((R/'reports/RESULTS_SUMMARY.json').read_text()); qc=json.loads((R/'reports/QC_REPORT.json').read_text()); dp=pd.read_csv(R/'tables/clinical_disproportionality.csv'); missing=pd.read_csv(R/'tables/clinical_missingness_summary.csv');top=s['top_absolute_feature_correlation']; cfg=json.loads((R/'config/clinical_phenotype_config.json').read_text())
# Narrative adjudication remains conservative; descriptive coefficients alone do not establish support.
correspondence='partially supported' if s['pair_rho']>0 and s['drug_label_permutation_tail_fraction']<.05 else 'indeterminate'
coverage='\n'.join(f'| {k} | {v}/10 |'+ ' / '.join(cfg['axes'][k])+' |' for k,v in s['axis_drug_coverage'].items())
rows='\n'.join(f'| {r.drug} | {r.total_drug_reports:,} | {r.nonmissing_features}/14 | {r.unstable_cells} |' for r in missing.itertuples())
ct=s['clinical_triazolam_minus_diazepam'];directions='\n'.join(f'| {t} | '+(f'{v:+.3f}' if v is not None else 'NA (a<5)')+' |' for t,v in ct.items())
text=f'''# 三層 working hypothesis 予備検証 — 2026-09-20
...
- **A. Clinical fingerprint feasibility: supported（取得方法の範囲内）**。10剤中{s['clinical_drug_count']}剤で安定性基準を満たす値を作成。6軸を保持し、14語を単一scoreへ統合しない。用語ごとの低countは下表参照。厳密なprimary-suspect cohort、完全な個票共変量・重複症例監査まで実現したという意味ではない。
- **B. X ↔ Y correspondence: {correspondence}**。{s['pair_count']} drug pairsの記述的Spearman rho={s['pair_rho']:.4f}。9999回の薬剤ラベル置換による両側tail fraction={s['drug_label_permutation_tail_fraction']:.4f}。45組を独立n=45とする検定はしていない。全estimable値を含める感度解析rho={s['all_estimable_sensitivity_rho']:.4f}、1剤除外rho範囲={s['leave_one_drug_out_rho_range'][0]:.4f}〜{s['leave_one_drug_out_rho_range'][1]:.4f}。
...
[openFDA drug event API](https://open.fda.gov/apis/drug/event/)のFAERS報告文書。`receivedate:[20040101 TO 20251231]`を全薬剤・背景で共用。背景は{qc['background_total']:,}文書。取得日時、完全query、HTTP status、レスポンスhashをrawに保存。APIの更新日時は各response metaに保存（本取得の背景responseは2026-07-30更新）。2026年の部分年は含めない。
...
**最大の記述的候補**: `{top['structural_feature']}` × `{top['phenotype_term']}`、rho={top['rho']:.4f}、n_effective={top['n_effective']}。これは672比較中の事後的な最大絶対値の記述であり、独立な発見・残基機序・再現性の保証ではない。同率候補は全表に残る。共通交絡や構造feature間相関によっても生じ得る。

## Diazepam / triazolam三層例

Xは全featureのtriazolam−diazepam差を表示。Mは同一研究、同一経口dose 2/5 mg/kg、同一time 15/30/60/90分の8context。Triazolamのrotarod failure fractionは{s['rotarod_triazolam_higher_contexts']}条件で高く、{s['rotarod_tied_contexts']}条件で同値。n=2薬剤であり、8条件を8薬剤対とは扱わない。Mの未報告値はNA。
...
'''
(R/'reports/FINAL_REPORT.md').write_text(text)
(R/'reports/EVALUATIONS.json').write_text(json.dumps({'A_clinical_representation':'supported','B_XY':correspondence,'C_M_data_linkage':'supported','D_working_hypothesis_feasibility':'indeterminate','M_intermediate_function':'indeterminate'},indent=2))
manifest=[]
for p in sorted((R/'raw/lineage').glob('*')):manifest.append({'path':str(p.relative_to(R)),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()})
(R/'config/lineage_manifest.json').write_text(json.dumps(manifest,indent=2))
